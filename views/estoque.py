...
def render(user):
    ...
    with aba_est4:
        ...
        with sub1:
            ...
            SELECT m.id, m.item_id, m.data_movimentacao, m.tipo_movimentacao, i.nome_material, m.quantidade,
            ...
            if not df_movs_todas.empty:
                ...
                with st.expander("✏️ Editar ou 🗑️ Excluir Lançamento de Movimentação", expanded=False):
                    dict_movs = {
                        f"Reg #{r['id']} | {r['Data']} - {r['tipo_movimentacao']} de {r['quantidade']:.0f} {r['unidade_medida']} ({r['nome_material']}) - Destino: {r['secao_destino']}": r
                        for _, r in df_movs_todas.iterrows()
                    }
                    mov_selecionada_txt = st.selectbox(
                        "Selecione o Lançamento para Alterar ou Excluir:",
                        list(dict_movs.keys()),
                        key="sel_mov_livro_edit"
                    )
                    dados_m = dict_movs[mov_selecionada_txt]
                    id_mov = int(dados_m['id'])
                    id_item = int(dados_m['item_id'])
                    tipo_mov = str(dados_m['tipo_movimentacao'])
                    qtd_antiga = float(dados_m['quantidade'])

                    tab_edit_m, tab_del_m = st.tabs(["✏️ Editar Movimentação", "🗑️ Excluir Registro"])

                    with tab_edit_m:
                        with st.form(f"form_editar_mov_{id_mov}"):
                            c_m1, c_m2 = st.columns(2)
                            nova_qtd = c_m1.number_input(
                                f"Quantidade Movimentada ({dados_m['unidade_medida']}):",
                                min_value=0.1,
                                value=qtd_antiga,
                                step=1.0
                            )
                            dt_val_obj = pd.to_datetime(dados_m['data_movimentacao']).date() if pd.notna(dados_m['data_movimentacao']) else datetime.now().date()
                            nova_dt = c_m2.date_input(
                            ...
                            c_s = conn.cursor()
                            c_s.execute("SELECT nome_secao FROM estoque_secoes WHERE om_id = ? ORDER BY nome_secao", (user['om_id'],))
                            secoes_banco = [r[0] for r in c_s.fetchall()]
                            if dados_m['secao_destino'] and dados_m['secao_destino'] not in secoes_banco:
                                secoes_banco = [dados_m['secao_destino']] + secoes_banco

                            idx_sec = secoes_banco.index(dados_m['secao_destino']) if dados_m['secao_destino'] in secoes_banco else 0
                            nova_secao = c_m3.selectbox("Seção / Destino:", secoes_banco, index=idx_sec)
                            novo_militar = c_m4.text_input("Militar Responsável:", value=dados_m['militar_responsavel'] or "")
                            nova_obs = st.text_input("Finalidade / Observação:", value=dados_m['observacao'] or "")

                            if st.form_submit_button("💾 Salvar Alterações na Movimentação", type="primary"):
                                diff = nova_qtd - qtd_antiga
                                cur = conn.cursor()
                                if tipo_mov == 'ENTRADA':
                                    cur.execute("UPDATE estoque_itens SET quantidade_atual = MAX(quantidade_atual + ?, 0.0) WHERE id = ?", (diff, id_item))
                                elif tipo_mov == 'SAÍDA':
                                    cur.execute("UPDATE estoque_itens SET quantidade_atual = MAX(quantidade_atual - ?, 0.0) WHERE id = ?", (diff, id_item))

                                cur.execute('''
                                UPDATE estoque_movimentacoes
                                SET quantidade = ?, data_movimentacao = ?, secao_destino = ?, militar_responsavel = ?, observacao = ?
                                WHERE id = ?
                                ''', (nova_qtd, str(nova_dt), nova_secao, novo_militar, nova_obs, id_mov))
                                conn.commit()
                                st.success(f"✅ Movimentação #{id_mov} atualizada com sucesso! O estoque foi recalculado.")
                                st.rerun()

                    with tab_del_m:
                        st.warning(f"⚠️ Atenção: Ao excluir este lançamento de **{tipo_mov}** ({qtd_antiga:.0f} {dados_m['unidade_medida']} de '{dados_m['nome_material']}'), o saldo físico do material será estornado automaticamente no Almoxarifado.")
                        confirma_del = st.checkbox("Confirmo que desejo excluir permanentemente este lançamento do livro.", key=f"chk_del_mov_{id_mov}")
                        if st.button("🗑️ Confirmar Exclusão Definitiva", type="primary", key=f"btn_exec_del_mov_{id_mov}"):
                            if not confirma_del:
                                st.warning("Por favor, marque a caixa de confirmação acima para autorizar a exclusão.")
                            else:
                                cur = conn.cursor()
                                if tipo_mov == 'ENTRADA':
                                    cur.execute("UPDATE estoque_itens SET quantidade_atual = MAX(quantidade_atual - ?, 0.0) WHERE id = ?", (qtd_antiga, id_item))
                                elif tipo_mov == 'SAÍDA':
                                    cur.execute("UPDATE estoque_itens SET quantidade_atual = quantidade_atual + ? WHERE id = ?", (qtd_antiga, id_item))

                                cur.execute("DELETE FROM estoque_movimentacoes WHERE id = ?", (id_mov,))
                                conn.commit()
                                st.success(f"🗑️ Movimentação #{id_mov} excluída e saldo físico de '{dados_m['nome_material']}' estornado com sucesso!")
                                st.rerun()
            else:
