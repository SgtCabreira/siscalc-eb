...
# Defina o nome do arquivo do banco de dados (ajuste se o nome do seu arquivo for diferente, ex: "sistema_militar.db")
            ...
            if not df_oms.empty:
                ...
                with st.form("form_edit_om"):
                    ...
                    if st.form_submit_button("Salvar Alterações da OM"):
                        try:
                            c.execute("UPDATE oms SET sigla = ?, nome = ?, ug = ? WHERE id = ?", (nova_sigla, novo_nome_om, nova_ug, id_om_edit))
                            conn.commit()
        ...
        with tab_om_theme:
            ...
            if not df_oms.empty:
                ...
                with st.form("form_theme_om"):
                    ...
                    arquivo_logo = st.file_uploader("Upload do Brasão / Distintivo da OM (PNG or JPG):", type=["png", "jpg", "jpeg"])
                    ...
                    if st.form_submit_button("Salvar Identidade Visual"):
                        ...
                        if arquivo_logo is not None:
                            ...
                            c.execute("UPDATE oms SET cor_primaria = ?, lema = ?, logo_base64 = ? WHERE id = ?", (nova_cor, novo_lema, logo_b64_new, id_om_th))
                        else:
                            c.execute("UPDATE oms SET cor_primaria = ?, lema = ? WHERE id = ?", (nova_cor, novo_lema, id_om_th))
                        conn.commit()
    ...
    with col_user:
        ...
        with tab_u_cad:
            if not df_oms_user.empty:
                with st.form("form_u"):
                    ...
                    perfis_escolhidos = st.multiselect("Perfis de Acesso Autorizados:", perfis_disponiveis_todos, default=["SALC"], key="cad_user_perf_multi")
                    ...
                    if st.form_submit_button("Cadastrar Militar"):
                        if not perfis_escolhidos:
                            ...
                            try:
                                c.execute("INSERT INTO usuarios (nome_guerra, posto_grad, identidade_militar, om_id, perfil, senha) VALUES (?, ?, ?, ?, ?, ?)", (n_guerra, p_grad, ident, dict_oms[om_u], perf_str, sen))
                                conn.commit()
        ...
        with tab_u_edit:
            if not df_usuarios.empty:
                opcoes_edit = {f"{row['posto_grad']} {row['nome_guerra']} - OM: {row['OM']} (ID: {row['identidade_militar']})": row['id'] for _, row in df_usuarios.iterrows()}
                ...
                with st.form("form_edit_user_tab"):
                    ...
                    novos_perfis_sel = st.multiselect("Perfis de Acesso Autorizados:", perfis_disponiveis_todos, default=perfis_atuais_u, key="ed_perf_multi")
                    ...
                    if st.form_submit_button("Salvar Todas as Alterações do Usuário"):
                        if not confirmar_alteracao:
                            ...
                            try:
                                ...
                                c.execute('''
                                    UPDATE usuarios SET posto_grad = ?, nome_guerra = ?, identidade_militar = ?, om_id = ?, perfil = ?, senha = ?
                                    WHERE id = ?
        ...
        with tab_u_del:
            if not df_usuarios.empty:
                opcoes_usuarios = {f"{row['posto_grad']} {row['nome_guerra']} - OM: {row['OM']} (ID: {row['identidade_militar']})": row['id'] for _, row in df_usuarios.iterrows()}
